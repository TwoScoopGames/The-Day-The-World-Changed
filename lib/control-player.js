...
function createRoom(entities, images) {
	var width = 20;
	var height = 12;
	...
	for (var x = 0; x < width; x++) {
		createWall(entities, image, x * wallWidth, 0 * wallHeight);
		if (x < doorStart || x > doorStart + doorWidth) {
			createWall(entities, image, x * wallWidth, (height - 1) * wallHeight);
		}
	}

	for (var y = 1; y < height - 1; y++) {
		createWall(entities, image, 0 * wallWidth, y * wallHeight);
		createWall(entities, image, (width - 1) * wallWidth, y * wallHeight);
	}
}
...
module.exports = function(ecs, data) {
	ecs.addEach(function(entity, elapsed) { // jshint ignore:line
		...
		if (!createdRooms) {
			createRoom(data.entities, data.images);
			createdRooms = true;
		}
