...
function addGib(entities, entity, images) {
	...
	gib.velocity = angleToVectors(Math.random() * Math.PI * 2, Math.random() + 0.5);
	gib.friction = {
		x: 0.97,
		y: 0.97
	};
	if (entity.regib !== undefined) {
		...
		gib.timers = {
			fadeAway: {
				...
				script: "./scripts/delete-old-entities"
			}
		};
	}
		...
		entity.collisions.forEach(function(collisionId) {
			...
			camera.timers = {
				shake: {
					...
					time: 0,
					max: 100,
					script: "./scripts/delete-shake"
				}
			};

			if (isConnectedToShark(data.entities.entities, entity, food) && (entity.timers === undefined || entity.timers.hurt === undefined || entity.timers.hurt.running === false)) {
